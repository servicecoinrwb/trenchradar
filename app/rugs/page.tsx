  ...
  return (
    <main className="min-h-screen bg-gray-950 text-white p-6">
      <div className="max-w-5xl mx-auto">
        ...
        <a href="/" className="text-gray-500 text-sm hover:text-green-400 transition-all mb-6 inline-block">Back to rankings</a>
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <a href="/report" className="bg-red-500 hover:bg-red-400 text-white font-bold px-5 py-2 rounded-xl text-sm transition-all">Report a Token</a>
        </div>
        ...
        <div className="mb-10">
          ...
          {topReported.length === 0 ? (
            ...
            <div className="space-y-2">
              {topReported.map((t, i) => (
                <a key={t.pair_address} href={`/token/${t.pair_address}`} className="grid grid-cols-5 items-center bg-gray-900 border border-gray-800 hover:border-red-800 rounded-xl px-4 py-4 transition-all">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {t.types.slice(0, 2).map((type: string) => (
                      <span key={type} className="text-xs bg-red-950 text-red-400 border border-red-900 px-2 py-0.5 rounded-full">{type}</span>
                    ))}
                  </div>
                  ...
                  <div className="text-gray-600 text-xs text-right">{new Date(t.latest).toLocaleDateString()}</div>
                </a>
              ))}
            </div>
          )}
            ...
            <div className="space-y-2">
              {rugs.map((rug) => (
                <div key={rug.id} className="grid grid-cols-5 items-center bg-red-950 border border-red-900 rounded-xl px-4 py-4">
                  ...
                  <div className="text-gray-600 text-xs text-right">{new Date(rug.confirmed_at).toLocaleDateString()}</div>
                </div>
              ))}
            </div>
          )}
